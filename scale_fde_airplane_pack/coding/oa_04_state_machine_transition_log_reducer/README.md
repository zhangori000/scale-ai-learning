...
You are given transition logs from many workflow runs.
Each row has:
- run_id
- from_state
- to_state
...
Implement a reducer that validates transitions and returns:
1. final state per run
2. invalid transition list with reasons
3. time spent in each state per run

Assume out-of-order arrival and duplicate rows.
Use deterministic ordering by (run_id, ts, original_index) after dedupe.
