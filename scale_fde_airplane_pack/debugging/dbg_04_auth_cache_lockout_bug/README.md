...
- failed auth decisions are cached
- requests check cache before backend verification
- after rotation/outage, valid users still receive `401` for several minutes
...
2. valid credential
3. temporary verifier/backend failure

If category 3 is cached as category 1, lockout occurs.

Debugging flow:

1. inspect cache entries for affected keys
2. compare lockout duration with negative TTL
3. correlate first lockout timestamp with backend health incidents
4. verify whether verifier exceptions are being mapped to negative cache writes

Patch policy:

- positive cache TTL moderate (for example 300s)
- negative cache TTL short (for example 10s)
- never negative-cache temporary backend errors
- invalidate auth cache on key rotate/create/revoke events
...
- short negative TTL limits harm for true invalid keys
- temporary failures no longer create long lockout shadows
- explicit invalidation removes stale positive/negative entries during key lifecycle changes

Regression tests:

1. invalid key -> fast repeated reject, then expiration
2. temporary backend error -> no negative cache insertion
3. rotated key becomes accepted immediately after backend recovery/invalidation

Operational counters:

- `auth_negative_cache_hit_total`
- `auth_temp_backend_error_total`
- `auth_cache_invalidation_total`
