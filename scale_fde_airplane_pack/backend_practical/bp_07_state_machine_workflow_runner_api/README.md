...
This backend practical is inferred from: scale-agentex/agentex/docs/docs/development_guides/state_machines.md.

Build a FastAPI service with endpoints to: create workflow run. step workflow. fetch run state. resume run from persisted snapshot.

Persist run state and transition history in SQL. Handle concurrency so two workers cannot step the same run at once.
