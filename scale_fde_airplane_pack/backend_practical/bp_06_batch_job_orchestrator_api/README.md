...
1. `POST /v1/jobs` to create job and return immediately
2. `GET /v1/jobs/{job_id}` to poll state
3. `POST /v1/jobs/{job_id}/cancel` to request cancellation
...
2. insert `QUEUED` row
3. enqueue job id
4. return job_id and queued status
...
- update progress
- check whether status became `CANCEL_REQUESTED`
- stop gracefully and transition to `CANCELED` if requested

Terminal transition rules:

- only one terminal status write allowed
- terminal jobs should reject further cancel attempts
...
- if job in `QUEUED` or `RUNNING`, set `CANCEL_REQUESTED`
- if terminal, return conflict or no-op according to API contract

Test plan:

1. create returns quickly and job appears queued
2. worker moves job through valid lifecycle
3. double worker pickup prevented by CAS transition
4. cancel request during run leads to canceled terminal state
5. terminal job cannot be canceled again

Operational metrics:

- queue depth
- pickup latency
- run duration by job type
...
1. queue wait time
2. run duration by job type
3. failure reason taxonomy
4. cancellation latency
